...
const visibleMenuAsideMain = computed(() =>
  menuAsideMain.filter((item) => {
    if (item.adminOnly) return authStore.role === 'admin'
    if (item.employeeOnly) return authStore.role === 'employee'
    return true
  }),
)
      ...
      >
        ...
        <NavBarItemPlain use-margin>
          <FormControl
            ...
            transparent
            borderless
            class="w-64 rounded-full bg-gray-100 dark:bg-slate-700"
          />
        </NavBarItemPlain>
      </NavBar>
      <AsideMenu
